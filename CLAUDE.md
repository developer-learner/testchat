...
|------|---------|-------------|
| 2026-09-09 | T9 resumption initially staged full contracts instead of D-136 changed entries, then omitted required scalar fields. The acceptance-only closeout first declared an invisible no-edit-only delta, then incorrectly overlapped changed_files and no_edit_files. Browser review disproved the inherited assumption that only catalog.js/app.js needed repair: chrome.js still bound removed dialogs. | Read contracts-merge.py's staging shape before refreeze; stage required files/erd_version/entry_points plus changed entries only. Acceptance-only ratification names the actual corrected contract and protects its file, without contradictory edit/no-edit declarations. Inspect every consumer of removed DOM IDs and browser page errors even after the frozen suite passes. Apply D-132 at the concrete-defect checkpoint; record direct repairs separately from coder output. T9 evidence: tasks/T9-completion-review.md. |
| 2026-09-02 | A child-side re-pin for the D-178 family-match fix was committed **half-done**. `d6137b4` advanced the template manifest (`scripts/.manifest-template` — added the `selftest_family_match.py` entry, re-hashed `validate-plan.py` to the family-aligned `80e50df…`) and added the selftest symlink into the blueprint, but left `.template-version` pinned at the OLD blueprint commit `6132185`. The two must move together: the pre-push **release-gate materializes `.template-version`'s pin** into a sibling worktree and resolves the child's relative selftest symlinks against it. Against `6132185` the manifest was doubly wrong — `selftest_family_match.py` doesn't exist there (dangling symlink → pytest collection error → "no tests ran"), and `validate-plan.py` resolves to the pre-fix `0416d26…` (manifest-drift). So the gate REFUSED the push. The commit's own message even said "re-pin … to blueprint 0e09467" while the pin it left in place said otherwise — the manifest was coherent against a plane the pin never selected. Not a gate bug; the gate correctly caught an incomplete re-pin. (Compounding it, the first amend attempt bumped `.template-version` alone and tripped the **pre-commit** manifest gate, because `.template-version` is itself tracked in `scripts/.manifest-project` and its recorded hash had to be regenerated in the same commit.) | A child-side re-pin is ONE atomic commit that advances `.template-version` and every manifest it feeds together: (a) `.template-version` pin → the new blueprint ref; (b) `scripts/.manifest-template` → the new plane's hashes + any new selftest entries/symlinks; (c) `scripts/.manifest-project` regenerated for the new `.template-version` hash (it tracks that file too). Precondition (D-177): only re-pin to a blueprint ref that is already on `origin/main` (verify `origin-Blueprint == adopted ref` before the bump). Verify BEFORE pushing that, at the NEW pin, every child→blueprint symlink resolves and every `.manifest-template` hash matches — because that is exactly the plane the release-gate will materialize. Meta-rule: the pin selects the plane the manifest is judged against; a manifest advanced without its pin (or vice-versa) is coherent against a plane no gate ever loads, and the release-gate will find it. Landed here as `87f8ce7` (d6137b4 amended: pin→`0e09467`, `.manifest-project` regenerated); release-gate green (545 passed), CI + check-drift green on origin. |
| 2026-09-02 | Two findings from the CEO's review of the T8/router incident. (1) **The root cause preceded the routing miss:** the T3 wrong-brief chain began when **v115 dropped the `## Coder briefs (verbatim)` section** that v107–v113 had carried. D-107 / M35 established TPM-authored verbatim briefs for subtle tasks (the v82 ERD-DELTA spec `a56660b`; `f21e47c` is the D-64-placement half of the same redesign, not the briefs) precisely because the 27B EM composes wrong briefs below the constraint-density line. With no verbatim brief, the EM composed T3's as "put the not-ready re-probe in the exception handler" — the wrong path (the 404 race arrives as a message-less `("error",)` item in the stream loop, never a raised exception). The originating defect was a spec-authoring lapse — an existing mechanism dropped — not the coder or the model. (2) **Framing/deflection in my own reporting:** I then offered the fixes — route small changes direct (D-132), consolidate before the stack bloats (D-107 retire-on-refresh + the 2026-08-03 lesson), keep verbatim briefs — as "levers for smoother future updates," which frames them as process gaps to close. All three already exist in the blueprint; every failure was a failure to APPLY them (the routing one mine, this session). Labeling my own application-misses as systemic improvements softens accountability and aims future effort at building what already exists. | (1) A behavioral delta with any non-trivial control-flow or placement decision carries a TPM-authored `## Coder briefs (verbatim)` brief for that task — never left to EM composition (D-107 restated). A dropped verbatim brief on a subtle task is the first thing to look for when a coder "faithfully builds the wrong thing." (2) Reporting discipline: before offering any fix as a "lever / improvement / what would make this smoother," grep `docs/DECISIONS.md` + this log for the mechanism. If it already exists, name it as an application/discipline failure (and whose), not a systemic gap — a rule already in the log presented as a new improvement is a misattribution that hides the real miss. Meta-rule: the process usually already has the answer; when it does, the honest finding is "not applied," never "missing." |
