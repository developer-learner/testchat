...
def test_registry_contains_expected_script_models():
    ...
    assert entry_0731["ready_timeout_attr"] == "DEEPSEEK_0731_READY_TIMEOUT_SECONDS"
    assert entry_0731["base_url"] == "http://127.0.0.1:8005"


def test_registry_0731_source_string_is_accepted_by_response_schema():
    """The response schemas' closed-set source Literal must accept the new id
    or the /api/v1/models and /api/v1/models/catalog handlers raise pydantic
    ValidationError as soon as a load_script_model call makes 0731 loaded.
    Test asserts the schemas construct cleanly with the new source value."""
    from src.api.models import ModelInfo, CatalogEntry

    info = ModelInfo(id="deepseek-v4-flash-0731", source="deepseek-v4-flash-0731")
    assert info.source == "deepseek-v4-flash-0731"

    entry = CatalogEntry(id="deepseek-v4-flash-0731", source="deepseek-v4-flash-0731", loaded=False)
    assert entry.source == "deepseek-v4-flash-0731"
