# ERD-DELTA v109 — router model: ownership pins registered (v108 follow-on)

Freeze context: standing spec v108. v107 introduced the router feature
(AC-170..AC-174) but carried its behavioral-ownership pins only in the
ERD-DELTA Test-to-file mapping prose; the plan gate reads
`contracts.test_mapping` (data, not prose), so the catalog task (T4) had no
registered acceptance signal and the EM's plan was rejected with empty tests
for `src/static/catalog.js`. v108 repaired the smoke/wiring shape; v109
registers every test-to-file pin of this milestone in the frozen
`contracts.test_mapping` data plane, whose keys are now all frozen node-ids
from `scripts/.approved/test-nodeids`. AC-170..AC-174 stand exactly as
frozen in v107; no application file changes; no test bytes change.

## Changed acceptance criteria

None. AC-170..AC-174 were introduced by v107 and remain in force unchanged;
this freeze alters acceptance *wiring* only (ownership pins moved into the
frozen data plane), never a criterion.
...
- `src/services/storage.py`, `src/api/threads.py`, `src/static/threads.js`,
  ...
  no code change.
- `scripts/.approved/contracts.json` (repair) — the milestone's twenty
  test-to-file pins (fifteen router tests, the catalog dropdown test, and the
  four carried-owner regressions) are registered in `contracts.test_mapping`;
  `smoke_checks` retains the standing three entries plus the v108 `threads.js`
  entry. No standing registration is altered or removed.
...
Every pin above is now also registered in the frozen `contracts.test_mapping`
data plane (this freeze's purpose); the ERD-DELTA section remains as the
freeze-time pin-gate and B3 synthesis source.
...
- Registration is safe now that every key exists as a frozen node-id in
  `scripts/.approved/test-nodeids` (the freeze-time family match accepts both
  bare and `[chromium]`-shaped keys; the keys above use the exact frozen
  shapes).
- The D-124 auto-placement rule now places each registered node-id at the task
  owning its file — T4's catalog test no longer depends on the EM's
  placement memory; the plan gate's per-task acceptance reads the registered
  mapping.
- `smoke_checks` entries for `src/static/threads.js` are green-on-unchanged
  ...
  behavioral probe.
- AC-170..AC-174 remain exactly as frozen in v107; v108/v109 are
