# ERD-DELTA v108 — router model: acceptance-wiring repair (v107 follow-on)

Freeze context: standing spec v107. v107 introduced the dual-path router
feature (AC-170..AC-174) but left its acceptance wiring incomplete: the
catalog UI pin used the bare node-id family while the frozen test-nodeids
carry the `[chromium]` parametrization (breaking the B3 mechanical plan
synthesis), `src/static/threads.js` carried no `smoke_check` in contracts
(leaving T7 without any acceptance signal), and the no-edit carried-owner
tasks T5..T8 had no mapped pins in this delta's Test-to-file mapping. This
freeze repairs all three without changing any behavior: AC-170..AC-174 stand
exactly as frozen in v107, no application file changes, no test bytes change.

## Changed acceptance criteria

None. AC-170..AC-174 were introduced by v107 and remain in force unchanged;
this freeze alters acceptance *wiring* only (mapping pins and a smoke check),
never a criterion.
...
- `src/services/storage.py`, `src/api/threads.py`, `src/static/threads.js`,
  ...
  behavior is pinned via this delta's Test-to-file mapping; no code change.
- `scripts/.approved/contracts.json` (repair) — `smoke_checks` gains one
  entry for `src/static/threads.js` (`grep -q '_enqueueMutation'
  src/static/threads.js`), giving the carried-owner task an acceptance signal
  where none existed. No standing smoke check is altered or removed.
...
The v108 pins are carried regressions (except the v107-introduced router
tests and the catalog dropdown test, also carried as byte-identical frozen
bytes): every inventory file owns at least one mapped acceptance node-id.
...
behavior stays pinned in this delta's mapping (the mapped regression test
...
No code change. In the inventory because this delta's mapping pins
`tests/test_data_safety_storage.py::test_valid_json_with_invalid_thread_schema_is_quarantined`
...
as behavioral owner, and contracts.json now carries the smoke check
...
- The v108 pins for carried files use the exact frozen node-ids from
  `scripts/.approved/test-nodeids` (including the `[chromium]`
  parametrization where the frozen ids carry it); the bare-family variants
  v107 used for the catalog test are corrected to the frozen shape so the B3
  mechanical plan synthesis resolves every milestone node-id.
- `smoke_checks` entries for `src/static/threads.js` are green-on-unchanged
  ...
  behavioral probe.
- AC-170..AC-174 remain exactly as frozen in v107; this freeze is a
  bookkeeping repair and must not re-price the milestone.
