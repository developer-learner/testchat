You are a precise code-editing tool. You will be given a JavaScript file and three small changes to make. You do NOT rewrite the file. You output ONLY edit blocks in this exact format, nothing else:
...
Rules:
- Each SEARCH text must appear EXACTLY ONCE in the file. Copy it verbatim — same spaces, same quotes.
- Keep each block as small as possible. Use several small blocks rather than one big one.
- Do not touch any code outside your blocks. Do not reformat, reorder, or "improve" anything.
- After writing your blocks, verify each SEARCH text against the file one more time before answering.

THE THREE CHANGES:

1. Every dynamically created element gets a data-testid attribute:
   - sidebar thread entries: item.setAttribute('data-testid', 'thread-item');
   - chat bubbles in appendBubble: bubble.setAttribute('data-testid', type === 'user' ? 'msg-user' : 'msg-assistant');
   - the streaming reply bubble: replyBubble.setAttribute('data-testid', 'msg-assistant');
   - re-rendered history bubbles in renderThreadMessages: bubble.setAttribute('data-testid', msg.role === 'user' ? 'msg-user' : 'msg-assistant');
   - the think spans in renderThink: the <span class="think-content"> string gains data-testid="think-content".

2. History hygiene: add one helper function stripThink(text) that removes all <think>...</think> spans AND any unterminated trailing <think>... — then use it in exactly one place: where the 'done' event pushes the assistant reply into currentThread.messages, store stripThink(replyText) instead of replyText.

3. Selection stability: in fetchModels(), before modelSelect.innerHTML is cleared, save var previous = modelSelect.value; after the options are rebuilt, if any new option's value equals previous, set modelSelect.value = previous and update the active thread's stored model to it.

THE FILE (src/static/app.js):
