...
- **Ledger back-port (D-160):** `docs/DECISIONS.md` now mirrors the blueprint's full D-1..D-157 set verbatim — the missing D-48 + D-56..D-106 (the drift the review flagged) plus D-150..D-157 landed in one scripted pass; a padded `comm` over entry numbers now shows no diff between the ledgers. The testchat-local script-paths entry (previously D-150) was renumbered to **D-158** with a note — one number names one decision.

- **Script-model paths env-ified (D-158, testchat-local):** `src/services/models.py` script paths are no longer hardcoded machine absolutes — `DS4_0731_SCRIPT_PATH` / `DS4_Q2KXL_SCRIPT_PATH` / `DS4_IQ3XXS_SCRIPT_PATH` / `NEMOTRON_SCRIPT_PATH` env overrides with the current absolute paths as defaults, so the frozen tests (which assert those defaults) stay green and another machine can point the engines elsewhere. (Committed as `a42076a`; originally numbered D-150, renumbered to D-158 by the back-port pass.)
