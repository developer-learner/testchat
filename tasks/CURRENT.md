...
- **Script-model paths env-ified (D-161, testchat-local):** `src/services/models.py` script paths are no longer hardcoded machine absolutes — `DS4_0731_SCRIPT_PATH` / `DS4_Q2KXL_SCRIPT_PATH` / `DS4_IQ3XXS_SCRIPT_PATH` / `NEMOTRON_SCRIPT_PATH` env overrides with the current absolute paths as defaults, so the frozen tests (which assert those defaults) stay green and another machine can point the engines elsewhere. (Committed as `a42076a`; originally numbered D-150, renumbered to D-158 then D-161 as the blueprint claimed those numbers.)

- **Child-owned scripts drift-covered (D-162):** `scripts/.manifest-project` now lists the three testchat-owned scripts that no manifest could see — `lane-selfcheck.py`, `run-server-0731-q2.sh`, `run-server-0731-ud.sh` (the same drift-invisible class the review flagged for the template's bootstrap/new-project). Template-owned `scripts/bootstrap.sh` + `scripts/new-project.sh` will enter the child's `.manifest-template` with the next template sync (blueprint-side already covered, D-158).

- **Scroll bug fixed + committed `56835db`** (`fix(ui): stick-to-bottom stream scroll; never yank past chats mid-stream`, pushed). Verified live by CEO and via full suite 214/214. Root cause: `queueRender()` (app.js) ran a 30ms timer on every stream token that unconditionally called `scrollToBottom()`, which forced `scrollTop = scrollHeight`; a still-running stream outlived thread switches and yanked the shared `#chat-container` to the bottom of whatever past chat was open. Fix: `scrollToBottom(force)` is stick-to-bottom (only repins while ≤24px off-bottom, app.js:96-105); the stream tick only auto-scrolls when the stream's thread is the one on screen (app.js:254-257); user send still force-scrolls (app.js:210). No frozen test pinned the behavior (only body horizontal-overflow at test_ui.py:511-514); 58 targeted + full 214-milestone suite green on host.
- **Answered: terminating testchat does NOT unload script models.** No shutdown/lifespan hook anywhere in `src/main.py` (no SIGINT/SIGTERM handling, no `atexit`); spawned engines are independent `Popen` children and get orphaned (still running, RAM held). Sidecars in `data/model-sidecars/` exist precisely so a restarted testchat can positively identify and Eject orphaned servers (models.py:101-108). Unload-all-on-exit is unimplemented.
- **Backlog added** (`tasks/BACKLOG.md`, `## Later`): (1) P2 — model add/unload issues: use a small fast test LLM and walk real user flows (dropdown load, send-while-streaming, Eject, eviction, engine crash, restart-mid-load) to catalog defects from repros, then fix; flagged "for later". (2) P1 — model router app: extract engine lifecycle (omlx/antirez ds4 0731, llama.cpp llama-server unsloth GGUFs, LM Studio, mtplx) out of `src/services/models.py` into a standalone router with a management surface and a universal OpenAI-compatible service (`GET /v1/models`, `POST /v1/chat/completions`, SSE) so testchat/OpenCode/the pi agent can consume loaded models without knowing engines. Feasibility check requested (universal-service claim).
- **Session backlog:** engine ports live: LM Studio :1234, mtplx :8080; ds4-deepseek-0731 :8005 and llama-server :8101/:8102 are loadable-but-currently-unloaded. Tests pass under `DS4_URL`-style isolation per the standing convention.

---

## State at 2026-08-09 — model-management bundle shipped as a CEO-directed direct fix; pipeline halt formally closed

- **CEO ruling (superseded by D-132, 2026-08-10):** routing between the milestone pipeline and a direct fix is decided by SIZE and DETERMINISM, never the bug/feature label — see `docs/DECISIONS.md` D-132. Original ruling: "the Lima VM + EM/coder + sandbox are for milestone feature runs ONLY, not ad-hoc bug fixes... Bug fixes now go direct — no refreeze/orchestrate — unless the CEO explicitly asks for the milestone path." That axis was wrong: v99 (a bug fix) belonged in the pipeline and sailed green; this bundle (also a bug fix) belonged direct — small and deterministic — but rode the pipeline because it was inherited as a spec-v100 milestone (the conductor's mistake) and died there for spec-authoring defects before shipping as `7bfc622`. Both paths carry the same tests-first bar; the direct path is the ceremony scaled down, not a lower tier.
...
- **D-132 (routing decision):** pipeline-vs-direct is decided by size and determinism, never the bug/feature label; supersedes the 2026-08-09 ruling bullet above. Documented in `docs/DECISIONS.md` (both ledgers) and the blueprint (`a0f6e35`); the testchat ledger was re-aligned in the same operation (D-130 back-port, D-131 restoration).
...
## Results
  ...
  Delta-mapped frozen tests green against spec v121 — feature done (verdict scope: mapped tests only, D-112). Feature built and validated.
