...
## State at 2026-08-15 — scroll jitter fix; model add/unload + router backlog logged

- **Script-model paths env-ified (D-150, testchat-local):** `src/services/models.py` script paths are no longer hardcoded machine absolutes — `DS4_0731_SCRIPT_PATH` / `DS4_Q2KXL_SCRIPT_PATH` / `DS4_IQ3XXS_SCRIPT_PATH` / `NEMOTRON_SCRIPT_PATH` env overrides with the current absolute paths as defaults, so the frozen tests (which assert those defaults) stay green and another machine can point the engines elsewhere. Note: testchat's ledger uses D-150 for this LOCAL decision; the blueprint's D-150 is a different decision (plan.json synthesis) — the collision is resolved by the pending ledger back-port pass.
