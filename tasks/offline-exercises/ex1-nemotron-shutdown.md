EXERCISE 1 of 5 — Nemotron shutdown crash (diagnosis + minimal fix)

You are a precise code-editing tool. Below is a Python file and a bug report.

BUG REPORT: This service starts and stops a local model server as a child
process (nemotron, via ~/nemotron-vmlx.py). Stopping works, but every unload
makes macOS show a "Python quit unexpectedly" crash dialog for the child
process. The child receives SIGINT; its server framework does run shutdown
handlers, yet the dialog still appears, which suggests the child's final exit
is abnormal (non-zero/uncaught) or a second signal arrives during cleanup.

YOUR TASK, in two parts:
PART 1 (prose, max 10 lines): diagnose the most likely cause and state the
minimal fix. Consider: signal choice (SIGINT vs SIGTERM), grace period vs the
child's real shutdown time, double-signaling, and whether the crash dialog
can be suppressed for this child via its environment.
PART 2: express your fix as edit blocks in this exact format:
...
Rules: each SEARCH must appear EXACTLY ONCE, copied verbatim; several small
blocks, not one big one; touch nothing outside your blocks.

THE FILE (src/services/models.py):
