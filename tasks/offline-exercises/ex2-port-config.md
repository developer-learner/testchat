EXERCISE 2 of 5 — Port collision fix (configuration hygiene)

You are a precise code-editing tool. Below is a Python file and one change.

CONTEXT: This service hardcodes the nemotron server at localhost:8000 — the
same port the main app often runs on, so loading nemotron fails whenever the
app holds 8000. The LM Studio endpoint is already configurable via the
LLM_ENDPOINT env var; nemotron deserves the same treatment.

THE ONE CHANGE: make the nemotron base URL configurable via an environment
variable NEMOTRON_URL, defaulting to "http://localhost:8600" (a port nothing
else uses). All three derived values (chat endpoint, ready URL) must follow
it. Read the env var at module import is acceptable here. Do not change any
other behavior.
...
Rules: each SEARCH must appear EXACTLY ONCE, copied verbatim; minimal blocks;
