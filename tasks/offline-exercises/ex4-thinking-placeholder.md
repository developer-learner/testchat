EXERCISE 4 of 5 — "Thinking..." placeholder while the model reasons silently

You are a precise code-editing tool. Below is a JavaScript file and one change.

CONTEXT: Some local models reason silently for minutes before their visible
answer starts. During that time the reply bubble is completely empty (the
reasoning is hidden behind a toggle), so a working app looks frozen.

THE ONE CHANGE: while a reply is in flight and the bubble has produced no
VISIBLE text yet, the reply bubble shows the placeholder text "thinking..."
(plain text). The placeholder must disappear the moment any visible (non-
think) content renders, and must not appear in stored history, and must not
break the existing think-span rendering. Hint: the cleanest seam is where
replyBubble.innerHTML is assigned from the accumulated text — if the rendered
result contains no visible text, show the placeholder instead; set the
initial bubble content to the placeholder right after the bubble is created.

Reply with ONLY edit blocks (format below). IMPORTANT: never include any line
containing a think tag in a SEARCH section — anchor on nearby tag-free lines.
In any new code, never write the think tag as one literal string — construct
it by concatenation ('<' + 'think>') if you must reference it.

<<<<<<< SEARCH
(EXACT copy of 3-10 existing lines)
=======
(the same section with your change)
>>>>>>> REPLACE
