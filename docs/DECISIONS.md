...
**Do not suggest:** Removing the pin gate to speed freezes (a slice without the gate is a silent-drop hazard); trimming `entry_points` against a hand-maintained module list (derivation is deterministic); shipping the full contracts to the DRIFT/SPEC-DEFECT consult (it judges the whole decomposition, D-116); authoring the backfill pins in the conductor lane.
