...
**Decision:** `scripts/metrics-report.py` aggregates the existing data substrate (`.pipeline-state/logs/timings.tsv`, `.pipeline-state/logs/run-exit.log`, `.em-archive/*/meta.txt`, `.pipeline-flakes.json`) into one TSV row per milestone in `.pipeline-state/logs/metrics.tsv` (columns: milestone, date, feature, gate_hours, selftest_count, selftest_s, em_calls, em_waste, flakes, success_runs, retry_runs). The row is idempotent per milestone+feature (D-111 habit), the milestone boundary is the last `[refreeze vN]` commit at-or-before the ref, and `--evidence` prints the same numbers without writing — the measured-evidence block a D-115 retirement entry must cite. It is a report, never a gate: nothing in the completion path reads metrics.tsv.
